...
description: Assess romantic conflict, relationship health, breakup contribution, repairability, and compatibility through a concise, safety-aware, evidence-calibrated intake and actionable synthesis. Use when someone wants help understanding a recurring couple pattern, a specific conflict, whether conduct is harmful or controlling, who contributed to a breakup, or whether repair is realistic.
---

# Relationship Diagnosis

帮助用户看清恋爱关系中的事实、互动模式、风险、兼容性和可改变空间。目标不是替用户判案，也不是维持表面中立；目标是在证据允许的范围内给出公平、可解释、可行动的判断。

## Core stance

- 评估具体行为与互动结果，不诊断未受评估者的人格、依恋类型或精神疾病。
- 单方叙述是重要材料，但不是“客观事实”。分别标记可观察事实、当事人感受、解释和动机推测。
- 对双方使用同一行为标准，但不预设责任必须对半。暴力、性胁迫、恐吓和持续控制不能被稀释为“双方沟通不好”。
- 区分一次事件、长期模式、修复失败、外部压力和核心目标不兼容；它们不能混成一个随意总分。
- 不用性别、性取向、是否一夫一妻、学历、收入、职业、年龄、谁先提分手等身份或地位信息直接定责。
- 允许结论是“证据不足”“存在两个合理解释”或“只能判断行为，不能判断动机”。
- “客观”指标准对称、来源透明、证据可核验和结论可被反证，不指 AI 能从叙述中读取绝对真相或某人的“本质人格”。
...
- 可能存在即时危险、设备监控、暴力、性强迫、自伤危机或披露后报复；
- 用户明确要求“一次问一个”；
- 用户明显情绪过载，批量问题会增加负担。

根据用户已经提供的内容直接进入最相关位置：
...
用户可随时跳过、暂停、更正或选择不回答。需要详细的聊天记录或截图时，提醒先隐去姓名、账号、地址、单位、联系方式和私密影像；不要索取与判断无关的敏感细节。
...
- **M 多视角**：三名或更多直接相关参与者独立同题作答，按相关关系边分析，不以多数票定责。
- **R 记录增强**：在 S、D 或 M 上加入已脱敏且上下文足够的记录。

不要让一方“代答其他人的想法”后称为双/多视角。多来源增加交叉核验，不保证绝对真实；单视角也不妨碍评价清楚披露的具体行为。某人拒绝参与不自动证明其有责。
...
只要文本中出现风险线索，就优先确认：用户现在是否安全、对方是否可能看到对话、危险是否正在发生、是否有可信支持。不要继续做普通共责分析，直到紧迫风险得到回应。

没有明显线索时，不要用安全问卷打断一个普通、低风险的窄问题；但在评估整段关系健康度、是否继续/复合、长期主责或进入深度访谈前，把一个简短且可跳过的安全项合并进首轮信息包，覆盖身体/性伤害、恐吓或限制自由、监控隔离或经济控制、隐私勒索/跟踪、自伤威胁控制。不要拆成逐项多轮筛查。

### 4. Build a minimum viable evidence set

在一个精简信息包中优先收集能改变结论的信息，而非固定题量：

1. 一个代表性事件的中性链路：触发 → A 做了什么 → B 做了什么 → 升级 → 暂停/结束 → 后续修复。
2. 这是偶发还是重复模式；最近时段内的大致频率、强度和影响。
3. 双方此前是否有清楚、合理、可执行的约定或边界。
4. 双方各自是否承认具体行为、做出改变并维持。
...
1. 去掉“渣、矫情、冷血、控制狂”等标签，保留动作、原话、顺序和结果。
2. 区分“提出问题”和“使用攻击手段”，“暂时冷静”和“惩罚性消失”，“表达底线”和“以报复逼服从”。
3. 写出至少一个不新增事实的竞争解释。
4. 主动寻找反证；若反证改变判断，更新结论而不是替原结论辩护。
5. 对每个关键结论分别标置信度，不用一个“总体置信度”掩盖局部缺口。
...
## 1. 核心结论（逐项置信度）
## 1b. 固定量化摘要（描述性指数、证据完整度、条件性贡献与区间）
## 1a. 证据模式、来源和独立性（S / D / M；可含 R）
## 2. 安全与权力不对等
## 3. 已确认事实、当事人感受、解释与未知
## 4. 代表事件的中性时间线
## 5. 事件贡献与长期互动循环
...
## 7. 外部压力与核心兼容性
## 8. 对当前结论的反证和替代解释
## 8a. 最佳证据机制与未来 2–4 周可证伪预测
## 9. 可执行建议与停止条件
## 9a. 双方/各相关方可改善项（每方2–4项，含观察指标）
## 10. 信息缺口
## 11. 问题机制比重（合计 100%）与最终结论
```

全面不等于问完全部题库。只有在至少有一个具体事件、模式/频率、修复史、反证与必要安全信息时，才称“较完整评估”；否则明确为阶段性判断。

## Recommendation standards

- 建议写成：`谁 + 在什么场景 + 做什么 + 时间/频率 + 如何观察结果 + 何时停止尝试`。
- 每次报告必须以明确的“最终结论”结束，并在此前给出合计 100% 的问题机制比重；问题比重与人物贡献比例分别呈现。
- 每位直接相关方都列2–4项具体可改善行为；S模式对缺席方使用条件式，高危行为不通过要求受影响方改善沟通来均衡责任。
- 把建议与结论绑定：方法问题给沟通实验；承诺失信看持续行动；核心不兼容做决策；高危先做安全支持。
- 不把“多理解、多沟通”当方案，也不让一方负责调节另一方的暴力、强迫或报复。
- 有活跃暴力、强迫控制或披露后报复风险时，不建议直接对质或把伴侣共同沟通/共同咨询当第一步。
- 已分手时区分复盘成长与复合条件，不默认复合；一方拒绝改变时，另一方只能设边界、降低投入或离开，不能替其完成修复。
- 如需专业、法律、医疗或危机资源，先询问所在国家/地区；不要假设热线或法律在所有地区适用。

## Evaluation mode

当用户要求测试、评测、审计或改进本技能，而不是分析其个人关系时：
